...
var RepositoriesListField = React.createClass({
  ...
  render: function() {
    ...
    _.each(this.props.repositories, function(repository) {
      repositories.push(<option key={repository.name} value={repository.name}>{repository.name} ({repository.path})</option>)
    });

    return (
      <div className="form-group">
        <label htmlFor={this.props.name} className="control-label">{this.props.title}</label>
        <select name={this.props.name} value={this.props.value} className="form-control" onChange={this.handleChange}>
          {repositories}
        </select>
      </div>
    );
